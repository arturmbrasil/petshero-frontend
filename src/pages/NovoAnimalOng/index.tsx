  ...
  const handleSubmit = useCallback(
    async (data: FormData) => {
      try {
        ...
        const response = await api.post('/ongs/animals', formData);

        if (response.data.id)
          history.push(`/animais-ong/editar/${response.data.id}`);

        addToast({
          type: 'success',
          title: 'Animal cadastrado!',
          description: 'Cadastramos as informações do seu animal!',
        });
        addToast({
          type: 'info',
          title: 'Experimente cadastrar uma foto!',
          description:
            'Cadastrar uma foto do seu animal vai ajudá-lo a achar um lar mais rapidamente!',
        });
      } catch (error) {
